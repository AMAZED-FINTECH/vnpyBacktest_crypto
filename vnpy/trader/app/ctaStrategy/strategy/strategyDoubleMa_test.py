...
class DoubleMaStrategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        self.bg_hour1 = BarGenerator(self.onBar, 60, self.onBar_hour1)
        self.am_hour1 = copy.deepcopy(ArrayManager(10))

        self.bg_hour4 = BarGenerator(self.onBar, 4, self.onBar_hour4, INTERVAL_60M)
        self.am_hour4 = copy.deepcopy(ArrayManager(10))
    ...
    def onBar(self, bar):
        ...
        if bar.interval == INTERVAL_1M:
            self.bg_hour1.update_bar(bar)

    # ----------------------------------------------------------------------
    def onBar_hour1(self, bar):
        print("收到60分钟数据from：" + str(bar.datetime_start) + " low " + str(bar.datetime_end))
        self.am_hour1.updateBar(bar)
        if not self.am_hour1.inited:
            return
        # time.sleep(0.5)
        self.cancelAll()


        am = copy.copy(self.am_hour1)
